 ...
 *  @warning    18/3/20  移植时，移植时要注意到，send_b，OutData[]，已经定义
 ******************************************************************************/
...
7.  移植时，移植时要注意到，send_b，OutData[]，已经在text.c文件定义*/
 void OutPut_Data_test(void)
 {
  ...
  OutData[1] = 12345;//adc_once(ADC1_SE13, ADC_12bit);
...
void OutPut_Data_test_sscom(void)
{
 ...
 // MessageProcessing();//电感采集
  
  int16 databuff[20];
  unsigned char l;
  send_b=10000;//给这个数赋值
  OutData[0] = ADC_Value[0];//(int)speed_forecast_left;//ADC_Value[0];//adc_once(ADC1_SE10, ADC_12bit);
  OutData[1] = ADC_Value[1];//speedctrl_left;//ADC_Value[1];//adc_once(ADC1_SE13, ADC_12bit);
    //var_test4 = adc_once(ADC1_SE13, ADC_12bit);
  OutData[2] = ADC_Value[2];//speed_now_left;//ADC_Value[2];//adc_once(ADC1_SE14, ADC_12bit);
  OutData[3] = ADC_Value[3];//(int)speed_fe;//ADC_Value[3];//adc_once(ADC1_SE15, ADC_12bit);
  //printf("ADC_FroBack_6[1]=%d\n",OutData[0]);
  for(l=0;l<5;l++)
  {  
...
/*******************************************************************************
 *  @brief      AB交换数据的函数
 *  @note       可放入main中while（1)里执行               
      
                以字节形式发送，超过字节大小，可以分成几个字节(用于停发车标识，)

 *  @warning   
 ******************************************************************************/
void nrf_exchange_data()
{
  uint8 nrf_databuff[32];
  uint8 len=1;
  nrf_databuff[0]=0x00;
  ...
  nrf_tx_mode();//此部分代码需要修改，还要nrf连接时序正确
  nrf_tx(nrf_databuff,len);
  nrf_rx_mode();
  nrf_rx(nrf_databuff,len);
}
